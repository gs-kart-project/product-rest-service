...
// Only active ProductIndexer when gskart.search.engine=elasticsearch (the local default) - a
// future OpenSearchProductIndexer would be gated the same way, both implementing the same port.
@Service
@ConditionalOnProperty(name = "gskart.search.engine", havingValue = "elasticsearch", matchIfMissing = true)
public class ElasticsearchProductIndexer implements ProductIndexer {

    private static final Logger log = LoggerFactory.getLogger(ElasticsearchProductIndexer.class);

    private final ElasticsearchOperations elasticsearchOperations;

    public ElasticsearchProductIndexer(ElasticsearchOperations elasticsearchOperations) {
